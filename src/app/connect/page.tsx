...
export default async function ConnectPage({ searchParams }: { searchParams: { error?: string } }) {
  ...
  // Only show portals this user has access to
  const userPortals = userId
    ? await prisma.userPortal.findMany({
        where: { userId },
        include: {
          portal: {
            select: {
  ...
  const existingPortals = userPortals.map((up) => up.portal);
      ...
      {existingPortals.length > 0 && (
        <div className="max-w-4xl w-full mx-auto px-6 mb-6">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            <h3 className="text-sm font-bold text-gray-700 mb-3">Your Portals</h3>
      ...
      <main className="flex-1 flex items-center justify-center px-4 pb-12">
        <div className="max-w-4xl w-full grid md:grid-cols-2 gap-12 items-center">
          ...
          <div>
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-8">
              ...
              <p className="text-sm text-gray-500 mb-6">
                {existingPortals.length > 0
                  ...
                  : "Connect your HubSpot portal and we'll map your workflows instantly. No credit card required."}
              </p>
              ...
              <a href="/api/auth/hubspot"
                ...
                <span className="flex items-center justify-center gap-2">
                  ...
                  Connect HubSpot{existingPortals.length > 0 && " Portal"}
              ...
              {!userId && (
                <p className="text-center text-xs text-gray-400 mt-3">
                  You{"'"}ll need to <Link href="/login" className="text-blue-600 hover:text-blue-700 font-medium">sign in</Link> or <Link href="/register" className="text-blue-600 hover:text-blue-700 font-medium">create an account</Link> first.
                </p>
              )}

              <div className="mt-6 pt-6 border-t border-gray-100">
                <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-3">Permissions requested</h3>
                <div className="space-y-2">
                  {[
            ...
            <p className="text-center text-xs text-gray-400 mt-4">Free for up to 10 workflows. No credit card required.</p>
          </div>
        </div>
      </main>
    </div>
