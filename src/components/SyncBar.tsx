...
export default function SyncBar({
  ...
  // ── Single polling loop — never restarts ──
  useEffect(() => {
    let active = true;
    ...
    const tick = async () => {
      ...
      try {
        ...
        // Phase transitions
        if (data.status === "SYNCING" && phaseRef.current === "idle") {
          setPhase("syncing");
        }
        if ((data.status === "COMPLETED" || data.status === "FAILED") && phaseRef.current === "syncing") {
          setPhase("done");
          // After 1.5s, go idle and notify parent
          setTimeout(() => {
            if (!active) return;
            setPhase("idle");
  ...
  }, [portalId]); // Only depends on portalId — never restarts

  // ── Cooldown for free tier ──
  useEffect(() => {
    if (!isFree || !lastSynced) { setCooldownMs(0); return; }
    const calc = () => {
  ...
  const isDisconnected = syncStatus?.status === "FAILED" && syncStatus?.message?.includes("disconnected");
    ...
    return (
      <div className={`flex items-center gap-3 px-4 py-3 bg-red-50 border border-red-200 rounded-xl ${compact ? "text-xs" : ""}`}>
        <div className="flex-1 min-w-0">
          <p className={`font-semibold text-red-900 ${compact ? "text-xs" : "text-sm"}`}>HubSpot Disconnected</p>
          {!compact && <p className="text-xs text-red-700 mt-0.5">Reconnect to resume syncing.</p>}
        </div>
        <Link href="/api/auth/hubspot" className="flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-orange-500 hover:shadow-md transition-all">
          Reconnect
        </Link>
      </div>
    );
          ...
          {!compact && (
            <div className="hidden sm:flex items-center gap-1 flex-shrink-0">
              {steps.slice(0, 6).map((s, i) => (
                <span key={s.label} className={`text-[9px] font-medium px-1.5 py-0.5 rounded transition-all duration-300 ${
  ...
  if (isFailed) {
    return (
      <div className={`flex items-center gap-3 px-4 py-2.5 bg-amber-50 border border-amber-200 rounded-xl ${compact ? "text-xs" : ""}`}>
        <div className="flex-1 min-w-0">
          <p className={`font-semibold text-amber-900 ${compact ? "text-xs" : "text-sm"}`}>Sync failed</p>
          {!compact && syncStatus?.message && <p className="text-xs text-amber-700 mt-0.5 truncate">{syncStatus.message}</p>}
        </div>
        <button onClick={triggerSync} className="flex-shrink-0 text-xs font-semibold text-amber-700 hover:text-amber-800 px-3 py-1.5 rounded-lg hover:bg-amber-100 transition-colors">
          Retry
        </button>
      </div>
    );
  ...
  return (
    <div className={`flex items-center gap-2 ${compact ? "px-3 py-1.5" : "px-4 py-2.5"} bg-gray-50 border border-gray-200 rounded-xl`}>
      ...
      <span className={`flex-1 ${compact ? "text-[10px]" : "text-sm"} text-gray-500 truncate`}>
        {onCooldown ? `Sync in ${formatCountdown(cooldownMs)}` : lastSynced ? `Synced ${timeAgo(lastSynced)}` : "Ready to sync"}
      </span>
      <button onClick={triggerSync} disabled={onCooldown}
        className={`flex-shrink-0 font-semibold rounded-md transition-colors ${compact ? "text-[10px] px-2 py-0.5" : "text-xs px-3 py-1.5"} ${onCooldown ? "text-gray-400 cursor-not-allowed" : "text-blue-600 hover:text-blue-700 hover:bg-blue-50"}`}>
        {onCooldown ? "⏳" : "Sync"}
      </button>
    </div>
  );
