...
export default function NavBar({ portalId, portalName }: NavBarProps) {
  ...
  return (
    <header className="h-14 border-b border-gray-200 bg-white flex items-center justify-between px-4 md:px-6 flex-shrink-0 relative">
                ...
                <div className="absolute top-full left-0 mt-1 w-64 bg-white rounded-xl shadow-lg border border-gray-200 py-1.5 z-40">
                  ...
                  <div className="border-t border-gray-100 mt-1 pt-1">
                    ...
                    <button onClick={async () => {
                      ...
                      setShowSwitcher(false);
                      try { await fetch("/api/portals/disconnect", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ portalId: pid }) }); router.push("/connect"); } catch {}
                    }}
      ...
      {/* Mobile hamburger */}
      {pid && (
        <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden p-2 -mr-2 text-gray-500 hover:text-gray-700">
          {mobileMenuOpen ? (
      ...
      {pid && mobileMenuOpen && (
        <>
          ...
          <div className="absolute top-14 left-0 right-0 z-40 bg-white border-b border-gray-200 shadow-lg md:hidden">
            {/* Portal name */}
            <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-emerald-500" />
              <span className="text-sm font-medium text-gray-700">{portalName || "Portal"}</span>
            </div>
            {/* Nav links */}
            <nav className="py-2">
              {links.map(link => {
                ...
                return (
                  <Link key={link.href} href={link.href} onClick={() => setMobileMenuOpen(false)}
                    className={`block px-4 py-2.5 text-sm font-medium transition-colors ${isActive ? "text-blue-700 bg-blue-50" : "text-gray-700 hover:bg-gray-50"}`}>
                    {link.label}
                  </Link>
                );
              })}
